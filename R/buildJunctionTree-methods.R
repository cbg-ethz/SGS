...
getLoopyGroupCliques <- function(BNdag)
  # Return list of nodes including their neighbors for a DAG
  # added for loopy belief propagation
{
  dimBN <- dim(BNdag)[1]
  groupCliques <- list()
  tempigraph <- graph.adjacency(BNdag, "undirected", weighted=NULL, diag=TRUE,
                           add.colnames=NULL, add.rownames=NA)
  for(jj in 1:dimBN){
    #print(c(as.integer(neighbors(tempigraph,jj,"all")),jj))
    groupCliques[[jj]] <- c(as.integer(neighbors(tempigraph,jj,"all")),jj)
  }
  return(groupCliques)
}
